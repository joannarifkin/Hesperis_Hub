...
#SBATCH --job-name=STAR_pass_1_Rifkin_Hesperis
...
#SBATCH --output=Hesperis_STAR_pass_1-%A-%a.log
#SBATCH --array=1-20
...
file=$(ls /nfs/turbo/rsbaucom/lab/Hesperis_transcriptome_assemblies_2022/Trim_galore_*/*R1_trimmed.fq.gz | sed -n ${SLURM_ARRAY_TASK_ID}p)
basefile=${file%_R1_trimmed.fq.gz}
samplename=$(basename "$file" _R1_trimmed.fq.gz)
...
--readFilesIn $basefile\_R1_trimmed.fq.gz $basefile\_R2_trimmed.fq.gz \
